...
mle_gamma_est <- function(x_vec){
  ...
  x_n = length(x_vec)
  x_prod = prod(x_vec)
  optimum_vals = optimize(neg_likelihood, interval = vals, x_n = x_n, x_prod = x_prod)
  optimum_vals
}
...
neg_log_likelihood <- function(alpha, x_sum, x_n){
  x_n * lgamma(alpha) - alpha * x_sum
}
...
simulate_one_eta <- function(n, alpha_sim, alpha_0){
  ...
  # calculate eta stat
  neg_ll_H1 = mle_vals$objective
  ll_ratio = - gamma(alpha_0)^(-n)*(prod(x_vec))^alpha_0 / neg_ll_H1
...
profvis({
  ...
  pwr_vec_unb = power_vec_calc(1e5, 1, 2, alpha_vec_unb, crit_unb)
