...
```{r}
dmodel <- function(x, n = 10, p = 0.5){
  print(n)
  print(p)
  dbinom(x, n, p)
}

pmodel <- function(x, n = 10, p = 0.5, tail_left = TRUE){
  pbinom(x, n, p, tail_left)
}

sample_space <- function(n = 10){
  0:n
}

expected_val_model <- function(n = 10, p = 10){
  n*p
}
...
```{r}
p_val_a <- function(x, pmodel, dmodel, ...){
  pr_tail_l = pmodel(x, ...)
  ## Use 1 - tail prob but add point prob
  pr_tail_r = 1 - pr_tail_l + dmodel(x, ...)
  
  p_val = 2 * min(pr_tail_l, pr_tail_r, 1/2)
  p_val
}

p_val_a(7, pmodel, dmodel, n = 10, p = 0.7)
```

```{r}
p_val_b <- function(x, dmodel, pmodel, sample_space, multi_modal = TRUE, ...){
  W_x = dmodel(x, ...)
  ...
  if (multi_modal){
    # If multimodal this would be a bad test p-method
    for(y in S){
      W_y = dmodel(y, ...)
      if(W_y > W_x){
      }
...
p_val_b(3, dmodel, pmodel, sample_space, multi_modal = TRUE, n = 10, p =0.3)
```
...
p_val_c <- function(x, n, p){
  ...
  p_tail_r = pmodel(x, n, p, tail_left = FALSE) + dmodel(x, n, p) #Fix this
  epsilon = 1e-8
...
comparison_p_vals <- function(n, p){
  ...
  for(i in 0:n){
    ...
    p_obs = c(p_obs, p_val_obs(i, n, p))
  }
  
  df <- data.frame(p_a, p_b, p_c, p_d, p_obs)
  rownames(df) <- 0:n
  
  df
}

comparison_p_vals(10, 0.5)
```
