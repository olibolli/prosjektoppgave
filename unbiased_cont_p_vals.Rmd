...
find_t_dual_and_calc_tail_prob <- function(t_obs, pT, dT, intT, E_T){
  ...
  if(t_obs < E_T){
    ...
    t_dual = tryCatch({uniroot(dual_equation_high, t_obs = t_obs, pT = pT, dT = dT, intT = intT, E_T = E_T,  interval = c(E_T+1e-4,100))$root
    }, error = function(e){
      return(E_T) # TODO : Why this value
    }) 
    # calculating tails
    p_val = 1 + pT(t_obs) - pT(t_dual)
    return(p_val)
    ...
    # Finding the 0 of the dual equation, t_dual
    t_dual = tryCatch({uniroot(dual_equation_low, t_obs = t_obs, pT = pT, dT = dT, intT, E_T = E_T,  interval = c(-100,E_T-1e-4))$root
    }, error = function(e){
      return(E_T) # TODO : Why this value
    }) 
    # Calculating tails
...
dPower_Gamma <- function(t, alpha_true, significance_value, pT, dT, intT, E_T){
  ...
  (significance_value >= unbiased_p_val_wip(t, pT, dT, intT, E_T)) * dlgamma(t, location = alpha_true, scale = 1)
