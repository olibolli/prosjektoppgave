...
```{r}
power_calc_binom_alt <- function(method, n, p_true, a, p_0 = .7){
  power = 0
  for(x in 0:n){
...
for(p in P){
  # Calculate power
  power_double = power_calc_binom_alt(p_val_a, n, p, .05)
  power_point = power_calc_binom_alt(p_val_b, n, p, .05)
  power_dist = power_calc_binom_alt(p_val_d, n, p, .05)
...
ggplot(data = df_power)+
  ...
  geom_line(aes(x = P, y = b_double), linetype = "dashed")
```
