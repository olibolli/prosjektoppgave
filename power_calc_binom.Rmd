...
ggplot(data = df_rej_area, aes(x = x, y = y, fill = value)) + 
  geom_tile()

plot(x = P, y = df_rej_size)
```
...
make_heatmap<- function(df){
  ...
  plot <- ggplot(melt_df, aes(x = x/sqrt(length(x)), y = y/sqrt(length(y)), fill = value))+
    geom_tile()+
    scale_fill_gradient(low = "white", high = "red", limits = c(-1, 1))+
    theme_classic()
  return(plot)
}
...
plotList[[2]] = NULL
plotList[[3]] = NULL
plotList[[4]] = make_heatmap(power_surf_a - power_surf_c)
plotList[[5]] = make_heatmap(power_surf_b - power_surf_c)
plotList[[6]] = NULL
plotList[[7]] = make_heatmap(power_surf_a - power_surf_d)
plotList[[8]] = make_heatmap(power_surf_b - power_surf_d)
plotList[[9]] = make_heatmap(power_surf_c - power_surf_d)

ggmatrix(plotList, 3, 3,
         xAxisLabels = c("Double tail", "Point", "Tail"),
         yAxisLabels = c("Point", "Tail", "Distance"))
```
